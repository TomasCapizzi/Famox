...
const useCodigo = (nombre, gas, conector, rango, modelo)=> { 
    ...
    function derivarFuncion(gasAcc){
         //console.log('nombre: ',nombre, gas, conector, rango, modelo);
        if(nombre.es === 'Caudalímetro'){
            return buscarCodigoCaudalimetro(gas, conector, rango)            
        } else if( nombre.es === 'Caudalímetro Compacto' ){
            return buscarCodigoCaudalimetroM3(gas, conector, rango)
        }        
        else if( nombre.es === 'Aspirador Venturi' ){
            return buscarCodigoAspiradorVenturi(gas, conector)
        } else if(nombre.es === 'Llave de Vacío' ){
            return buscarCodigoLlaveVacio(conector, modelo.nombre)
        } else if(nombre.es === 'Regulador de Baja Presión' ){
            return buscarCodigoReguladorBaja(gas, conector)
        } else if(nombre.es === 'Regulador de Vacío' ){
            return buscarCodigoReguladorVacio(conector, rango)
        } else if(nombre.es === 'Humidificador' ){
            return buscarCodigoHumidificador(gas)
        } else if(nombre.es === 'Regulador de Presión Ajustable' ){
            return buscarCodigoReguladorAjustable(gas, conector)
        } else if(nombre.es === 'Regulador de Presión Pre-Ajustado' ){
            return buscarCodigoReguladorPreajustado(gas, conector)
        } else if(nombre.es === 'Regulador de Presión Pre-Ajustado con Caudalímetro'){
            return buscarCodigoReguladorPreajustadoCaudalimetro(gas, conector, rango)
        } else if(nombre.es === 'Recipiente Colector' || nombre.es === 'Mezclador de Aire/Oxígeno'){
            return buscarCodigoDeModelo(modelo)
        } else if(nombre.es === 'Regulador de Baja Presión con Caudalímetro'){
            return buscarCodigoRegBajaCaudalimetro(gas, conector)
        } 
        else if(nombre.es === 'Máscara Nasal'){
            return buscarCodigoMascaraNasal(modelo.nombre)
        } else if(nombre.es.includes('U. Terminal')){
            return buscarCodigoUnidadTerminal(nombre,gasAcc)
        } else if(nombre.es.includes('Conector recto roscado')){
            return buscarCodigoConectorEntrada(nombre,gasAcc)
        }
        else if(nombre.es.includes('manguera')){
            return buscarCodigoConectorManguera(nombre,gasAcc)
        }
        else if(nombre.es.includes('Toma doble')){
            return buscarCodigoTomaDuplas(nombre,gasAcc)
        }
        else if(nombre.es.includes('bloque base')){
            return buscarCodigoAdaptacion(nombre,gasAcc)
        }
        else if(nombre.es.includes('Gabinete Expuesto')){
            return buscarCodigoKit(nombre, gasAcc)
        }
        else if(modelo.nombre.es === 'Positiva' || modelo.nombre.es === 'Negativa'){
            return buscarCodigoAlarmaGases(modelo)
        }
        else if(modelo.nombre.es === '16 Puestos' || modelo.nombre.es === '24 Puestos'){
    ...
    const buscarCodigoDeModelo = (modelo)=> {
        ...
        return modelo.nombre.es;
    }

    //ACCESORIOS
    // Unidades Terminales
    const buscarCodigoUnidadTerminal = (nombre, gas)=>{
        if(nombre.es === 'U. Terminal M2 DISS'){
        ...
        if(nombre.es === 'U. Terminal M2 AFNOR'){
            if(gas.toUpperCase() === 'OXIGENO'){
                return 'PUT072'
            } 
            else if(gas.toUpperCase() === 'AIRE'){
        ...
        if(nombre.es === 'U. Terminal M2 SS/AGA'){
            if(gas.toUpperCase() === 'OXIGENO'){
                return 'PUT017'
            } 
            else if(gas.toUpperCase() === 'AIRE'){
        ...
        if(nombre.es === 'U. Terminal M1 DISS'){
            if(gas.toUpperCase() === 'OXIGENO'){
                return 'PUT001'
            } 
            else if(gas.toUpperCase() === 'AIRE'){
    ...
    const buscarCodigoConectorEntrada = (nombre, gas)=>{
        if(nombre.es === 'Conector recto roscado DISS'){
            if(gas.toUpperCase() === 'OXIGENO'){
                return 'SCO001'
            } 
            else if(gas.toUpperCase() === 'AIRE'){
        ...
        if(nombre.es === 'Conector recto roscado AFNOR'){
            if(gas.toUpperCase() === 'OXIGENO'){
                return 'SCO010'
            } 
            else if(gas.toUpperCase() === 'AIRE'){
        ...
        if(nombre.es === 'Conector recto roscado SS/AGA'){
            if(gas.toUpperCase() === 'OXIGENO'){
                return 'SCO017'
            } 
            else if(gas.toUpperCase() === 'AIRE'){
        ...
        if(nombre.es === 'Conector recto roscado OHMEDA'){
            if(gas.toUpperCase() === 'OXIGENO'){
                return 'SCO028'
            } 
            else if(gas.toUpperCase() === 'AIRE'){
        ...
        if(nombre.es === 'Conector recto roscado ON'){
            if(gas.toUpperCase() === 'OXIGENO'){
                return 'SCO024'
            } 
            else if(gas.toUpperCase() === 'AIRE'){
        ...
        if(nombre.es === 'Conector recto roscado IRAM'){
            if(gas.toUpperCase() === 'OXIGENO'){
                return 'SCO040'
            } 
            else if(gas.toUpperCase() === 'AIRE'){
        ...
        if(nombre.es === 'Conector recto roscado YUGO'){
            if(gas.toUpperCase() === 'OXIGENO'){
                return 'SCO044'
            } 
            else if(gas.toUpperCase() === 'AIRE'){
    ...
    const buscarCodigoConectorManguera = (nombre, gas)=>{
        if(nombre.es === 'Conector recto para manguera DISS Plástico'){
            if(gas.toUpperCase() === 'OXIGENO'){
                return 'SCO123'
            } 
            else if(gas.toUpperCase() === 'AIRE'){
        ...
        if(nombre.es === 'Conector recto para manguera DISS Bronce'){
            if(gas.toUpperCase() === 'OXIGENO'){
                return 'SCO048'
            } 
            else if(gas.toUpperCase() === 'AIRE'){
        ...
        if(nombre.es === 'Conector recto para manguera DISS'){
            if(gas.toUpperCase() === 'OXIGENO'){
                return 'SCO126'
            } 
            else if(gas.toUpperCase() === 'AIRE'){
        ...
        if(nombre.es === 'Conector 90° para manguera DISS'){
            if(gas.toUpperCase() === 'OXIGENO'){
                return 'SCO057'
            } 
            else if(gas.toUpperCase() === 'AIRE'){
        ...
        if(nombre.es === 'Conector recto para manguera AFNOR'){
            if(gas.toUpperCase() === 'OXIGENO'){
                return 'SCO135'
            } 
            else if(gas.toUpperCase() === 'AIRE'){
        ...
        if(nombre.es === 'Conector 90° para manguera AFNOR'){
            if(gas.toUpperCase() === 'OXIGENO'){
                return 'SCO066'
            } 
            else if(gas.toUpperCase() === 'AIRE'){
    ...
    const buscarCodigoTomaDuplas = (nombre, gas)=>{
        if(nombre.es === 'Toma doble: Entrada DISS Salida DISS'){
            if(gas.toUpperCase() === 'OXIGENO'){
                return 'PUT048'
            } 
            else if(gas.toUpperCase() === 'AIRE'){
        ...
        if(nombre.es === 'Toma doble: Entrada DISS Salida AFNOR'){
            if(gas.toUpperCase() === 'OXIGENO'){
                return 'PUT088'
            } 
            else if(gas.toUpperCase() === 'AIRE'){
        ...
        if(nombre.es === 'Toma doble: Entrada AFNOR Salida AFNOR'){
            if(gas.toUpperCase() === 'OXIGENO'){
                return 'PUT057'
            } 
            else if(gas.toUpperCase() === 'AIRE'){
        ...
        if(nombre.es === 'Toma doble: Entrada AFNOR Salida DISS'){
            if(gas.toUpperCase() === 'OXIGENO'){
                return 'PUT079'
            } 
            else if(gas.toUpperCase() === 'AIRE'){
        ...
        if(nombre.es === 'Toma doble: Entrada O.N. Salida DISS'){
            if(gas.toUpperCase() === 'OXIGENO'){
                return 'PUT130'
            } 
            else if(gas.toUpperCase() === 'AIRE'){
        ...
        if(nombre.es === 'Toma doble: Entrada SS/AGA Salida DISS'){
            if(gas.toUpperCase() === 'OXIGENO'){
                return 'PUT134'
            } 
            else if(gas.toUpperCase() === 'AIRE'){
    ...
    const buscarCodigoAdaptacion = (nombre, gas)=>{
        if(nombre.es === 'Conexión DISS M1 a bloque base AFNOR M1'){
            if(gas.toUpperCase() === 'OXIGENO'){
                return 'SUT096'
            } 
            else if(gas.toUpperCase() === 'AIRE'){
        ...
        if(nombre.es === 'Conexión DISS M1 a bloque base O.N.'){
            if(gas.toUpperCase() === 'OXIGENO'){
                return 'SUT114'
            } 
            else if(gas.toUpperCase() === 'AIRE'){
        ...
        if(nombre.es === 'Conexión DISS M1 a bloque base AGA'){
            if(gas.toUpperCase() === 'OXIGENO'){
                return 'SUT120'
            } 
            else if(gas.toUpperCase() === 'AIRE'){
    ...
    const buscarCodigoKit = (nombre, gas)=>{
        if(nombre.es === 'Kit Gabinete Expuesto U.T.DISS'){
            if(gas.toUpperCase() === 'OXIGENO'){
                return 'KIT058'
            } 
            else if(gas.toUpperCase() === 'AIRE'){
        ...
        if(nombre.es === 'Kit Gabinete Expuesto U.T. AFNOR'){
            if(gas.toUpperCase() === 'OXIGENO'){
                return 'KIT065'
            } 
            else if(gas.toUpperCase() === 'AIRE'){
